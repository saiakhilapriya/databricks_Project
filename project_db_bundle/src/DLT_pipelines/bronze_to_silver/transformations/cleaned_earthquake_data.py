...
catalog_name = spark.conf.get('catalog_name','saiakhila_dev')
...
def earthquake_data():
    df = (
        ...
        .load(volume_path).withColumn("_load_time",current_timestamp())
    )
    df = df.withColumn("parsed_data", from_json(col("features"), schema))
    df = df.select(explode(col("parsed_data")).alias("features"),"_load_time")
    df = df.select(
        "features.properties.*",
        "features.id",
...
dlt.apply_changes(target= "earthquake_data_final",source = "earthquake_data_vw",keys = [primary_key],sequence_by = "_load_time",stored_as_scd_type = '1')